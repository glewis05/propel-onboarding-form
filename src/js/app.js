 ...
 */
function verifyEmailForDraft(formData, email) {
...
function FormWizard({ formDefinition }) {
    ...
    // RESUME MODAL STATE
    // =========================================================================
    // Controls the Resume Modal that allows users to select a previous draft
    // from a list and resume by verifying their email address.
    const [showResumeModal, setShowResumeModal] = React.useState(false);

    // Ref to track when a resume operation is in progress or just completed.
    // This prevents the email-watching effect from triggering checkForExistingDraft
    // after a successful resume (which would cause a duplicate RestorePrompt).
    // Using a ref instead of state to avoid stale closure issues with the timeout.
    const resumeInProgressRef = React.useRef(false);
    ...
    // =========================================================================
    // AUTO-RESUME ON MOUNT - DISABLED
    // =========================================================================
    // Previously, this effect checked localStorage on page load and automatically
    // showed a "Resume Previous Session?" prompt. This has been DISABLED because:
    //
    // 1. Form should always start BLANK for new users
    // 2. Resume flow must be USER-INITIATED via "Resume Your Onboarding Form" button
    // 3. User must select a specific clinic from draft list
    // 4. User must verify identity with email before loading data
    //
    // The proper flow is now:
    //   User clicks Resume button → ResumeModal → Select clinic → Verify email → Load draft
    //
    // React.useEffect(() => {
    //     try {
    ...
    // =========================================================================
    // EMAIL-BASED SESSION RESTORE - DISABLED
    // =========================================================================
    // Previously, this automatically detected when a user entered an email and
    // checked Supabase for existing drafts, showing a "Resume?" prompt.
    // This has been DISABLED because:
    //
    // 1. Auto-detecting drafts bypasses the explicit Resume flow
    // 2. Users should click "Resume Your Onboarding Form" button to see drafts
    // 3. Users must select a specific clinic and verify with email
    // 4. Starting a new form should not trigger prompts about old drafts
    //
    // The checkForExistingDraft function and email-watching effect are commented
    // out to prevent any automatic draft detection.
    //
    // const checkForExistingDraft = React.useCallback(async (email) => {
    //     if (!email || supabaseDraftId || resumeInProgressRef.current) return;
    //
    ...
    // Restore handlers
    const handleRestore = () => {
        // Set flag BEFORE restoring to prevent email-watching effect
        // from re-triggering checkForExistingDraft
    ...
    // =========================================================================
    // Called when user selects a draft from ResumeModal and verifies their email.
    // Restores the form data, step position, and Supabase draft ID.
    const handleResumeRestore = (restoredData) => {
        debugLog('[Resume] Restoring draft:', restoredData);

        // Set flag BEFORE restoring data to prevent email-watching effect
        // from triggering checkForExistingDraft (which would show duplicate popup)
        resumeInProgressRef.current = true;
        ...
        // Set Supabase draft ID for continued auto-save
        if (restoredData.submission_id) {
            setSupabaseDraftId(restoredData.submission_id);
        }

        // Update last saved timestamp for UI feedback
    ...
    return (
        <div className="max-w-3xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
            ...
            {/* Resume Modal - for selecting and resuming a previous draft */}
            {showResumeModal && (
                <ResumeModal
            ...
            {/* Resume Form Button - prominent placement for returning users */}
